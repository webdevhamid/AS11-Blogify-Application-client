...
const ArticleTemplate = ({ title, imageURL, id }) => {
  return (
    ...
    >
      ...
      {/* Article Tags */}
      {/* <span className="bg-primary text-white font-bold text-xs px-2 py-1 absolute right-0 top-0">
        {singleTag}
      </span> */}
      {/* <div className="flex">
        {tags?.map((tag, i) => (
          <span
            key={i}
            className="bg-primary text-white font-bold text-xs px-2 py-1 absolute right-0 top-0"
          >
            {tag}
          </span>
        ))}
